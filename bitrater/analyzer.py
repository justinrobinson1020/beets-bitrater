...
class AudioQualityAnalyzer:
    ...
    Uses deep learning (CNN + BiLSTM) by default when bundled model files are
    available, with SVM as fallback. Both classifiers can be configured via
    constructor arguments.
    """

    def __init__(self, model_path: Path | None = None, use_dl: bool = True):
        """
        Initialize analyzer components.

        Args:
            model_path: Optional path to pre-trained SVM classifier model
        ...
        # Try to load DL inference pipeline
        ...
        if use_dl:
            try:
                ...
                logger.debug(f"DL model not available, using SVM: {e}")

    def analyze_file(self, file_path: str) -> AnalysisResult | None:
        """
        Analyze a single audio file.

        Uses deep learning pipeline when available, otherwise falls back to
        SVM classification with cutoff detection and confidence penalties.
        """
        ...
        # 2. Try DL inference first
        if self._dl_pipeline is not None:
            result = self._analyze_file_dl(
                file_path, path, file_format, stated_bitrate, stated_class,
            )
            if result is not None:
                return result
            logger.debug(f"DL inference failed for {file_path}, falling back to SVM")

        # 3. Fall back to SVM pipeline
        return self._analyze_file_svm(
            file_path, path, file_format, metadata, stated_bitrate, stated_class,
        )
    ...
    def _analyze_file_svm(
        ...
        """Analyze using the SVM pipeline (original method)."""
        from .types import AnalysisResult

        # Extract spectral features with VBR metadata
        is_vbr = 0.0
        ...
        # Cutoff detection for validation
